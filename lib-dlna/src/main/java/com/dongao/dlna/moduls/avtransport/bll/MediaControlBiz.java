...
import com.dongao.dlna.moduls.avtransport.callback.avtransport.GetMediaInfo;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.GetPositionInfo;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Next;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Pause;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Play;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Previous;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Seek;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.SetAVTransportURI;
import com.dongao.dlna.moduls.avtransport.callback.avtransport.Stop;
import com.dongao.dlna.moduls.avtransport.callback.renderingcontrol.GetMute;
import com.dongao.dlna.moduls.avtransport.callback.renderingcontrol.GetVolume;
import com.dongao.dlna.moduls.avtransport.callback.renderingcontrol.SetMute;
import com.dongao.dlna.moduls.avtransport.callback.renderingcontrol.SetVolume;
import com.dongao.dlna.moduls.avtransport.constants.MediaControlWhat;
import com.dongao.dlna.moduls.avtransport.entity.MediaInfo;
...
public class MediaControlBiz {
	 ...
	 * @param url
	 */
	public void setPlayUri(String url) {
		upnpBiz.execute(new SetAVTransportURI(instanceId, serviceAVT, url,
			...
			public void failure(ActionInvocation invocation,
				...
				Log.d(TAG, "setPlayUri failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "SetAVTransportURI successed:" + defaultMsg);
	...
	public void getMediaInfo() {
		upnpBiz.execute(new GetMediaInfo(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "GetMediaInfo failure:" + defaultMsg);
			}
	...
	public void play() {
		upnpBiz.execute(new Play(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Play failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Play successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void pause() {
		upnpBiz.execute(new Pause(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Pause failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Pause successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void next() {
		upnpBiz.execute(new Next(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Next failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Next successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void previous() {
		upnpBiz.execute(new Previous(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Previous failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Previous successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void stop() {
		upnpBiz.execute(new Stop(instanceId, serviceAVT) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Stop failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Stop successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void seek(String totalTime, int percent) {
		...
		upnpBiz.execute(new Seek(instanceId, serviceAVT, seekTo) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "Seek failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "Seek successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void getMute() {
		upnpBiz.execute(new GetMute(serviceRC, instanceId) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "GetVolume failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(boolean currentMute) {
				Log.d(TAG, "GetVolume successed:current mute = "
						+ currentMute);
				Message msg = Message.obtain(handler);
	...
	public void setMute(boolean desiredMute) {
		upnpBiz.execute(new SetMute(serviceRC, instanceId, desiredMute) {
			...
			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "SetMute successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
	...
	public void getVolume() {
		upnpBiz.execute(new GetVolume(serviceRC, instanceId) {
			...
			public void onSuccess(int currentVolume) {
				Log.d(TAG, "GetVolume successed:current volume = "
						+ currentVolume);
				Message msg = Message.obtain(handler);
	...
	public void setVolume(int volume) {
		upnpBiz.execute(new SetVolume(serviceRC, instanceId, volume) {
			...
			public void failure(ActionInvocation invocation,
                                UpnpResponse operation, String defaultMsg) {
				Log.d(TAG, "SetVolume failure:" + defaultMsg);
			}

			@Override
			public void onSuccess(String defaultMsg) {
				Log.d(TAG, "SetVolume successed:" + defaultMsg);
				Message msg = Message.obtain(handler);
